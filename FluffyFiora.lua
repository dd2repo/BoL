...
function CastR()
	if Rready and ValidTarget(target, 400) and m.keysettings.forcer then
		CastSpell(_R, target)
	end
end
...
function Autoult()
	if m.ultimatesettings.useau then
		if m.ultimatesettings.auv == 1 then
			if CountEnemyHeroInRange(600) >= 2 then
				CastSpell(_R, target)
			end
		...
		elseif m.ultimatesettings.auv == 3 then
			if CountEnemyHeroInRange(600) >= 4 then
				CastSpell(_R, target)
			end
		elseif m.ultimatesettings.auv == 4 then
			if CountEnemyHeroInRange(600) >= 5 then
				CastSpell(_R, target)
			end
		end
	end
